...
|---------|------|---------|
| **v1.1** | 2026-05-26 | Added Feature Documentation skills to both levels (Category 3): Junior — Feature Documentation Hygiene + Feature Flag Documentation. Senior — Feature Documentation Ownership + Feature Flag Documentation Standards. Added "Documentation discipline" section to Tips (Section 8) with concrete examples (`claimsProcessingEnabled`, `SLA_VERSIONING_ENABLED`, `enable_require_id_scan`). |
...
- [1. Welcome & Your Probation Timeline](#1-welcome--your-probation-timeline)
- [2. Junior QA — Your 5 Goal Areas](#2-junior-qa--your-5-goal-areas)
- [3. Senior QA — Your 7 Goal Areas](#3-senior-qa--your-7-goal-areas)
- [4. How You'll Be Assessed](#4-how-youll-be-assessed)
- [5. Evidence That Counts](#5-evidence-that-counts)
- [6. Pass / Not Pass Thresholds](#6-pass--not-pass-thresholds)
- [7. What Happens at Each Outcome](#7-what-happens-at-each-outcome)
- [8. Tips to Succeed During Probation](#8-tips-to-succeed-during-probation)
...
|-----------|------|--------------|
| **Day 1** | Your first day | Onboarding kickoff. Your QA Lead walks you through this document. You receive Welcome Kit + 2-Week Onboarding Plan. Start setting your own goals. |
...
| **Day 45** | Pre-final check | Light check-in. Confirm you're on track. If gaps exist, agree on focus areas for final 2 weeks. |
...
> Post a brief daily update in the QA channel every working day. Format: what done today, what blockers, what's planned tomorrow. This is how your QA Lead stays informed without having to chase you — and how the team identifies issues early.
>
...
| **Postman Basics** | Can use QA Master Collection for simple requests. Refreshes tokens correctly. |
...
| Skill | Expectation |
|-------|-------------|
| **Test Case Writing** | Writes basic test cases using Jitsu template. Covers happy path. May miss edge cases — that's OK at this level. |
| **QMetry Usage** | Creates folder per ticket. Imports test cases. Follows naming convention. |
| **Verification Comments** | Posts comment per env (Staging / Beta / Prod) with status + evidence. May need reminders on build version. |
| **Bug Reporting** | Logs bugs per template (10 required fields). Mobile bugs include Fix Version. Evidence is clear. |
| **Release Process Awareness** | Understands isolated-release-branch concept at high level. Knows the rule to smoke test previous tickets on same repo. |
| **Feature Documentation Hygiene** | When working on a feature: searches Confluence for existing feature doc first. If exists → updates it to reflect new behavior. If not → asks Lead whether to write new (bonus if proactive). Doesn't let docs go stale. |
...
| **Production Safety** | Knows 8 Production Rules. Always asks supervisor before doing anything on Prod. |
...
| Skill | Expectation |
|-------|-------------|
| **Ticket Completion** | Completes assigned tickets within sprint. Ticket scope adjusted to Junior level. |
...
| Skill | Expectation |
|-------|-------------|
| **Daily Progress Reports to Lead** | Posts daily updates in QA channel so QA Lead always knows your status. Reports include: what done today, what blockers, what's planned tomorrow. NEVER skip — if no progress, still post and explain why. |
...
| **Mobile App Testing** | Tests Driver App across iOS + Android devices. Familiar with Firebase (Staging build distribution) + TestFlight (Prod build distribution) workflow. Can guide juniors through mobile testing process. |
...
Senior is expected to demonstrate mastery of QA process and define test approach.

| Skill | Expectation |
|-------|-------------|
| **Test Strategy** | Defines test approach per ticket. Identifies risks early. Adjusts test scope based on impact. |
| **Test Case Quality** | Test cases cover happy + edge + negative + integration scenarios systematically. |
| **QMetry & Documentation** | Maintains QMetry structure clean. Contributes to test case library for re-use. |
...
| **Feature Documentation Ownership** | Owns feature documentation discipline. For every feature shipped: ensures Confluence doc exists + is up to date. Writes new docs when none exist. Trains juniors to do the same. Treats stale docs as a quality issue, not just a nice-to-have. |
| **Feature Flag Documentation Standards** | Writes high-quality Confluence docs for every new feature flag: name, location (Consul / Firebase / `item_metadata`), default value, enable/disable conditions, scope (region / client / global), rollback plan, expected behavior when flag flips. Reviews juniors' flag docs. |
| **Production Safety** | Follows 8 Production Rules religiously. Trains junior on Prod safety. |
...
| Skill | Expectation |
|-------|-------------|
| **Ticket Completion Rate** | Completes assigned tickets on-time ≥ 90%. Handles complex tickets. |
| **Defect Escape Rate** | < 10% bugs found AFTER QA verified. Solid coverage. |
| **Test Coverage Depth** | Covers happy + edge + negative + performance considerations. |
| **Bug Triage** | Logs bugs with correct priority. Distinguishes regressions vs new issues. |
...
| **Process Improvement** | Suggests improvements to QA process. Implements when approved. |
| **Decision Making** | Makes judgment calls on test scope and priority. Can justify decisions to Engineering Lead with risk reasoning. |
...
| Skill | Expectation |
|-------|-------------|
| **Daily Progress Reports** | Posts daily updates in QA channel — even though no one asks. Lead always knows where Senior's work stands without having to chase. Senior models the reporting culture for juniors. |
| **Proactive Issue Escalation** | When hitting an issue (blocker, risk, environment problem, ticket scope confusion), raises to Lead immediately — not at next 1:1. Includes proposed solutions, not just problems. |
...
| **Quality Advocacy** | Pushes for quality at planning / refinement stages — not just at QA stage. |
| **Business Mindset (not Outsource Mindset)** | Doesn't just complete what's in the ticket. Thinks about the business goal — why does this feature matter to the client / driver / recipient? Suggests changes when ticket scope doesn't match business goal. |
...
> The scoring framework gives a data-driven view. Final decision involves QA Lead + Engineering Lead + HR — numbers alone don't capture everything. Borderline cases (just above/below threshold) are discussed carefully before the decision is final.
...
- You + QA Lead discuss next 3-month growth focus: which skills to deepen, what new areas to explore.
...
> ⚠️ **If you feel uncertain about your trajectory**
>
...
- Post daily progress reports in QA channel from Day 1 — it's a habit, not a punishment. Lead needs visibility.
...
- Bring questions to Day 45 check-in: what should you focus on?
...
- **When working on a feature** — first search Confluence for existing feature doc. If found, update it as part of your ticket. If not, ask Lead if you should write one (it's almost always yes).
...
- **Always verify** — AI can hallucinate Jitsu-specific terminology (fake driver types, wrong statuses, made-up API names). Cross-check against Confluence / code / asking team.
- **Give AI context** — paste the ticket, the AC, relevant code, the Confluence page. AI without context = generic answers.
- **Iterate** — first prompt rarely perfect. Refine: "Make it more specific to Jitsu's `NEXT_DAYS` flow" / "Focus on edge cases only" / "Output as markdown table".
